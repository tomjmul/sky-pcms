...
(async function () {
    ...
    records.forEach(csvLine => {
        const peacockSport = csvLine.sport.trim()
        const leagueName = (csvLine.league.trim() === "N/A") ? "null" : csvLine.league.trim()
        const appleSportName = csvLine.appleSport.trim()
        const appleLeagueAbbr = csvLine.appleLeague.trim()
        if (mapping[peacockSport] == null) {
            mapping[peacockSport] = {}
        }
        mapping[peacockSport][leagueName] = {
